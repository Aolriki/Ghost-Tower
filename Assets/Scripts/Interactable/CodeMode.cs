...
///     repassado para o CodeSlot que estiver ativo
/// </summary>
public class CodeMode : MonoBehaviour
{
    public static CodeMode Instance { get; private set; }
    ...
    void Update()
    {
        ...
        if (_inputHeld && _activeSlot != null)
            _activeSlot.HandleNavigate(_heldInput, isHeld: true);
    }
    ...
    public void EnterPropView(Transform camAnchor)
    {
        ...
        if (camAnchor == null)
        {
            Debug.LogWarning("[CodeMode] camAnchor nao atribuido no CodeSlot.");
            return;
        }
    ...
    // ── Public API: slot ativo ────────────────────────────────────────────────

    public void Activate(CodeSlot slot)
    {
        _activeSlot = slot;
        _selectedSlot = 0;
        _isActive = true;
    ...
    {
        if (!_isActive || _activeSlot == null) return;

        if (context.started)
        {
            _heldInput = context.ReadValue<Vector2>();
            _inputHeld = false;
            _activeSlot.HandleNavigate(_heldInput, isHeld: false);
        }
    ...
    {
        if (!_isActive || !context.performed || _activeSlot == null) return;
        _activeSlot.HandleConfirm();
    }

    public void OnCodeExit(InputAction.CallbackContext context)
    {
        if (!_isActive || !context.performed || _activeSlot == null) return;
        _activeSlot.HandleExit();
    }
